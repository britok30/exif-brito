...
export async function PhotoLibraryPage({ params, visibility }: { params: LibraryParams; visibility: LibraryVisibility }) {
  ...
  if (!(await auth())?.user) redirect(`/sign-in?callbackUrl=${encodeURIComponent(path)}`);
  ...
  return <PhotoViewerProvider photos={entries.map(viewerPhoto)}><main id="main" tabIndex={-1} className="archive-page">
    <GalleryHeader />
    <StudioIntro title={visibility === 'hidden' ? 'Unpublished photographs.' : visibility === 'published' ? 'Published photographs.' : 'Your photographs.'} note={visibility === 'published' ? <>Your photographs,<br />on view.</> : <>Choose your favorites.<br />Publish when you’re ready.</>}
      actions={<><Link href="/admin" className="studio-button">Dashboard</Link><Link href="/admin/upload" className="studio-button primary">Add photographs</Link></>} />
    <section className="archive-content photo-library" aria-label="Photo library">
      ...
      <header className="archive-toolbar"><p>{selection.visibility === 'hidden' ? 'Unpublished photographs' : selection.visibility === 'published' ? 'Published photographs' : 'All photographs'}</p>
        <span className="archive-total">{selection.start}–{selection.end} of {selection.total} / {hidden} unpublished</span><ViewSwitcher /></header>
      ...
      {selection.pageCount > 1 && <nav className="library-pagination" aria-label="Photo library pages">
        ...
        <form action={path} className="library-page-jump">
          ...
          <input key={selection.page} id="library-page" name="page" type="number" inputMode="numeric" min={1} max={selection.pageCount} defaultValue={selection.page} required />
          <span>of {selection.pageCount}</span><button className="studio-button" type="submit">Go</button>
        </form>
